...
try:
    ...
    print("ERROR: PIL (Pillow) is not installed. Install with: pip install Pillow")
    sys.exit(1)
...
def generate_icons_for_app(developer, app_name, config):
    ...
    for mipmap_name, size in ICON_SIZES.items():
        ...
        try:
            ...
            print(f"⚠️  Warning: Failed to create {mipmap_path}/ic_launcher.png: {e}")
            # Create a simple colored square as fallback
            fallback_img = Image.new('RGB', (size, size), color)
            fallback_img.save(os.path.join(mipmap_path, "ic_launcher.png"), "PNG")
            print(f"✅ Created fallback icon: {icon_path} ({size}x{size})")
        ...
        for developer, apps in APP_ICONS.items():
            ...
            for app_name, config in apps.items():
                ...
                try:
                    ...
                    error_msg = f"Error generating icons for {developer}/{app_name}: {str(e)}"
                    print(f"  ❌ {error_msg}")
